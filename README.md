...
## Calibrated_models
This repo includes the setup of all models based on the best parameter set attained through calibration. The sims only run on Linux. The time period of the simulation is from 02/12/2016 to 31/12/2019 (calibration period). The models are run by changing directory to the folder where the model files are located (cd …/FCR-GLM-metrics/Calibrated_models/Deepm*) and running the following command: ./glm+_latest/glm+.
...
## Validation
This repo includes the setup of all models based on the best parameter set attained through calibration. The sims only run on Linux. The time period of the simulation is from 20/07/2015 to 30/11/2016 (validation period). The models are run by changing directory to the folder where the model files are located (cd …/FCR-GLM-metrics/Validation/valid_*) and running the following command: ./glm+_latest/glm+.
...
- Obs_SS.csv - Observed Schmidt stability data calculated from the observed temperature profile and bathymetry data by ‘R-scripts/Observed_EXM.R’.
- obs_td.csv - Observed thermocline depth data calculated from the observed temperature profile data by ‘R-scripts/Observed_EXM.R’.
- mom_observed.csv - Observed metalimnetic oxygen minima data calculated from the observed dissolved oxygen profile data by ‘R-scripts/Observed_EXM.R’.
- anoxia_observed.csv - Number of anoxic layers calculated from the observed dissolved oxygen profile data by ‘R-scripts/Observed_EXM.R’.
...
This folder includes the setup of the uncertainty analysis using pestpp-ies, a model-independent iterative smoother. The uncertainty analysis repo includes two runs (Run1, Run2) that have different noise formulations. The following commands start the uncertainty analysis:
1. Changing directory to the 'Run2 folder' (cd .../FCR-GLM_metrics/Uncertainty_analysis/Run2).
...
- Figure8_uncertainty_plot.R - This script creates fan plots to visualise the prediction uncertainty of the Schmidt stability, the thermocline depth, and the metalimentic oxygen minima pre and post calibration (‘Results/Figure 8’). In this script, predictive uncertainty of the second pestpp-ies run is visualised. The uncertainty analysis is performed on the PEST_exm_w2 model with the deep mixing configuration set to 2.
...
- Noise_calculation_Run2.R - This script includes the calculation of the noise for the second uncertainty analysis run based on half of the maximum difference between the posterior mean of the first uncertainty analysis run and observations for each observation group. 
