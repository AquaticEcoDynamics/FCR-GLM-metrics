...
setwd(".../FCR-GLM-metrics/Calibrated_models/PEST_EXM_weight3_deepm1")
sim_folder <- getwd()
...
for(i in 1:length(unique(watertemp$Depth))){
  ...
  write.csv(match[[i]]$modtemp, paste0(".../FCR-GLM-metrics/Calibrated_models/PEST_EXM_weight3_deepm1/", #file path for saving csv files, should be set to the working directory
                                       "match", depths[i],
                                       ".csv"), row.names=FALSE)
}
...
for(i in 1:length(unique(oxy_compare$Depth))){
  ...
  write.csv(round(match_oxy[[i]]$mod_oxy, 8), paste0(".../FCR-GLM-metrics/Calibrated_models/PEST_EXM_weight3_deepm1/", #file path for saving csv files, should be set to the working directory
                                                    "matchoxy", depths[i],
                                                    ".csv"), row.names=FALSE)
